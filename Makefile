...
#--------------------------------------------------------------------
# This is a little experimental I guess. The idea is to have some real
# live recorded attestations available to use in our test suite.
# If the recordings from https://github.com/enterprise-contract/hacks/provenance/recordings
# change, you can use `make sync-test-data` to sync the changes to this local rego file.
RECORDED_TEST_DATA=policy/lib/slsa/tekton/test_data.rego

# See also https://github.com/enterprise-contract/hacks which you need
# to have checked out locally for this to work.
HACKS_DIR=../hacks
RECORDINGS_DIR=$(HACKS_DIR)/provenance/recordings

# Clears the file and adds the package command
_init-test-data:
	@echo Initializing $(RECORDED_TEST_DATA)
	@( \
...
_test-data-%:
	...
	@( export F="$*" && echo -n "att_$${F//-/_} := " && jq . $(RECORDINGS_DIR)/$*/attestation.json ) >> $(RECORDED_TEST_DATA)
	@opa fmt --write $(RECORDED_TEST_DATA)
...
sync-test-data: _init-test-data _sync-test-data-01 _sync-test-data-05 ## Refresh policy/lib/slsa/tekton/test_data.rego
	@echo Done updating $(RECORDED_TEST_DATA)
