...
test_image_registry_valid {
	deny_set := deny with input.attestations as prepare_mock_attestation_data(good_image)
	count(deny_set) == 0
}

# A invalid image registry
test_attestation_type_invalid {
	deny_set := deny with input.attestations as prepare_mock_attestation_data(bad_image)
	count(deny_set) == 1
