...
deny[{"msg": msg}] {
	some tr
	signed_annotation := data.cluster.TaskRun[tr].metadata.annotations["chains.tekton.dev/signed"]
...
all_annotations_present {
	every tr in data.cluster.TaskRun {
...
all_urls_match {
	url_base_set := {url_base |
		url := data.cluster.TaskRun[_].metadata.annotations["chains.tekton.dev/transparency"]
...
all_transparency_log_entries_are_present {
	every tr in data.cluster.TaskRun {
		...
		data.rekor[rekor_host].index[log_index].entry
	}
}
...
all_transparency_log_entries_appear_sane {
	...
	every tr in data.cluster.TaskRun {
		...
		rekor_host := split(url, "/")[2]
		log_index := split(url, "=")[1]
		log_entry_data := data.rekor[rekor_host].index[log_index].entry
