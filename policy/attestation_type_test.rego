...
test_attestation_type_valid {
	deny_set := deny with input as prepare_mock_attestation_data("https://in-toto.io/Statement/v0.1")
	count(deny_set) == 0
}

test_attestation_type_invalid {
	deny_set := deny with input as prepare_mock_attestation_data("https://in-toto.io/Statement/v6.283")
	count(deny_set) == 1
