...
test_good_dockerfile_param if {
	lib.assert_equal(set(), deny) with input.attestations as [_attestation({"parameters": {"DOCKERFILE": "./Dockerfile"}})] with data["task-bundles"] as lib.bundles.bundle_data
}
...
test_dockerfile_param_https_source if {
	...
	lib.assert_equal(expected, deny) with input.attestations as [_attestation({"parameters": {"DOCKERFILE": "https://Dockerfile"}})] with data["task-bundles"] as lib.bundles.bundle_data
}
...
test_dockerfile_param_http_source if {
	...
	lib.assert_equal(expected, deny) with input.attestations as [_attestation({"parameters": {"DOCKERFILE": "http://Dockerfile"}})] with data["task-bundles"] as lib.bundles.bundle_data
}
...
_attestation(params) = attestation if {
	attestation := {"predicate": {
		...
		"buildConfig": {"tasks": [{
			"name": "buildah",
			"ref": {"kind": "Task", "name": "buildah", "bundle": lib.bundles.acceptable_bundle_ref},
			"invocation": params,
		}]},
	}}
}
