package hermetic_pre_build_script_test

import rego.v1

import data.hermetic_pre_build_script
import data.lib

test_hermetic_pre_build_scripts if {
	lib.assert_empty(hermetic_pre_build_script.deny) with input.attestations as [_good_attestation]
}

test_not_hermetic_pre_build_scripts if {
	expected := {{
		"code": "hermetic_pre_build_script.pre_build_script_hermetic",
	...
	lib.assert_equal_results(expected, hermetic_pre_build_script.deny) with input.attestations as [hermetic_not_true]
	...
	lib.assert_equal_results(expected, hermetic_pre_build_script.deny) with input.attestations as [hermetic_invalid]

	# regal ignore:line-length
	hermetic_missing := json.remove(_good_attestation, ["/statement/predicate/buildConfig/tasks/0/invocation/parameters/HERMETIC"])
	lib.assert_equal_results(expected, hermetic_pre_build_script.deny) with input.attestations as [hermetic_missing]
}
