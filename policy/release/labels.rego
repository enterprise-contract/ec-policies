...
# description: >-
#   The image manfest is not accessible.
# custom:
...
# description: >-
#   The parent image manfest is not accessible.
# custom:
...
deny contains result if {
	...
	digest := manifest.annotations["org.opencontainers.image.base.digest"]
	parent_ref = sprintf("%s@%s", [name, digest])

	parent_manifest := ec.oci.image_manifest(parent_ref)
	is_null(parent_manifest)
	result := lib.result_helper(rego.metadata.chain(), [parent_ref, input.image.ref])
}
...
deny contains result if {
	...
	parent_name := manifest.annotations["org.opencontainers.image.base.name"]
	parent_digest := manifest.annotations["org.opencontainers.image.base.digest"]
	parent_ref = sprintf("%s@%s", [parent_name, parent_digest])

	parent_manifest := ec.oci.image_manifest(parent_ref)
	config_digest := object.get(parent_manifest, ["config", "digest"], "")
	config := ec.oci.blob(sprintf("%s@%s", [parent_name, config_digest]))
	is_null(config)
	result := lib.result_helper(rego.metadata.chain(), [parent_ref, input.image.ref])
}
...
_image_labels := labels if {
	...
	ref := image.parse(input.image.ref)
	config_ref := sprintf("%s@%s", [ref.repo, manifest.config.digest])
...
_parent := {"ref": ref, "config": config} if {
	manifest := ec.oci.image_manifest(input.image.ref)
	...
	parent_manifest := ec.oci.image_manifest(ref)

	config_ref := sprintf("%s@%s", [name, parent_manifest.config.digest])
