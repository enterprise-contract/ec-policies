...
#   Ensure the images for the presence of unpinned image references
# custom:
#   short_name: unpinned_related_images
#   failure_msg: The %q related image reference is not pinned with a digest.
#   solution: >-
...
deny contains result if {
	...
	result := lib.result_helper_with_term(rego.metadata.chain(), [unpinned_related_images], unpinned_related_images)
}
...
_related_images_not_in_snapshot contains unmatched_image_refs if {
	snapshot_components := input.snapshot.components
	component_images_digests := [component_image.digest |
		some component in snapshot_components
		component_image := image.parse(component.containerImage)
