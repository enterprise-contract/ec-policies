...
# This check only makes sense for index images?
# Get a list of all the RPMs.

# METADATA
# title: Unique Version
# description: >-
#   Check if there is more than one versions of the same RPM installed across different
...
deny contains result if {
	...
	exceptions := lib.rule_data("non_unique_rpm_names")

	some name, versions in grouped_rpm_purls
	count(versions) > 1
	not name in exceptions
