...
four_mock_empty_data := [lib.att_mock_helper("NOT_HACBS_TEST_OUTPUT", {}, "task1")]

test_needs_non_empty_data {
	lib.assert_equal(deny_test_data_missing, {{
		"code": "test_data_missing",
		"msg": "No test data found",
		"effective_on": "2022-01-01T00:00:00Z",
	}}) with input.attestations as four_mock_empty_data
}
...
test_needs_tests_with_results {
	lib.assert_equal(deny_test_results_missing, {{
...
test_success_data {
	lib.assert_empty(deny_test_data_missing) with input.attestations as mock_a_passing_test
	lib.assert_empty(deny_test_result_failures) with input.attestations as mock_a_passing_test
	lib.assert_empty(deny_test_result_unsupported) with input.attestations as mock_a_passing_test
	lib.assert_empty(deny_test_results_missing) with input.attestations as mock_a_passing_test
	lib.assert_empty(deny_test_result_unsupported) with input.attestations as mock_a_passing_test
	lib.assert_empty(warn_test_result_skipped) with input.attestations as mock_a_passing_test
}

mock_a_failing_test := [lib.att_mock_helper(lib.hacbs_test_task_result_name, {"result": "FAILURE"}, "failed_1")]

test_failure_data {
	lib.assert_equal(deny_test_result_failures, {{
...
test_can_skip_by_name {
	lib.assert_empty(deny_test_result_failures) with input.attestations as mock_mixed_data
...
test_skipped_is_warning {
	...
	lib.assert_equal(warn_test_result_skipped, {{
...
test_unsupported_test_result {
	...
	lib.assert_equal(deny_test_result_unsupported, {
