...
_supported_uris_reg := {
	"file:sha256-[0-9a-f]{64}",
	"file:sha384-[0-9a-f]{96}",
	"file:sha512-[0-9a-f]{128}",
}
...
#   failure_msg: >-
#     Code tampering detected, unacceptable task %q was included in build
...
deny contains result if {
	...
	chain_str := concat(", ", dependency_chain)

	some unacceptable_task in bundles.unacceptable_task_bundle(chain)
	unacceptable_task_name := tkn.pipeline_task_name(unacceptable_task)

	result := lib.result_helper_with_term(
		rego.metadata.chain(),
		[unacceptable_task_name, chain_str],
